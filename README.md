...
With the sidebar focused on a file row, or with the whole file open, switching
versions sets every chunk in that file to the selected version, so each chunk
you advance to opens in it.
...
The inspect suite's pinned-commit integration needs a checkout of oil.nvim and
some environment variables. See
[`.github/workflows/tests.yml`](.github/workflows/tests.yml) for the exact setup.
