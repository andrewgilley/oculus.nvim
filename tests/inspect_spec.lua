...
do
  ...
  assert(commit_config.width == main_config.width - 1)
  assert(commit_config.height == main_config.height - 1)
  ...
  assert(pull_request_config.width == main_config.width - 1)
  assert(pull_request_config.height == main_config.height - 1)
...
if integration_root and (integration_sha or integration_url) then
  ...
  do
    ...
    vim.api.nvim_win_set_height(
      overview_win,
      require("oculus.window").window_config({}).height - 1
    )
  ...
  do
    ...
    assert(overview_saved.config.width
      == main_overview_config.width - 1)

    assert(overview_saved.config.height
      == main_overview_config.height - 1)
