...
do
  local ctx_module_name = "treesitter-context.context"
  local original_ctx = package.loaded[ctx_module_name]

  local fake_ctx = {
    get = function(win)
      local buf = vim.api.nvim_win_get_buf(win)
      local state = vim.b[buf].oculus_inspect
      if state and state.role == "parent" then
        return { { 0, 0, 1, 0 } }, { "local function my_function()" }
      end
      return {}, {}
    end,
  }

  package.loaded[ctx_module_name] = fake_ctx
  inspect._mirror_parent_treesitter_context()

  local p_buf = vim.api.nvim_create_buf(false, true)
  local c_buf = vim.api.nvim_create_buf(false, true)
  vim.bo[p_buf].filetype = "lua"
  vim.bo[c_buf].filetype = "lua"
  vim.api.nvim_buf_set_lines(p_buf, 0, -1, false, {
    "local function my_function()",
    "  local a = 1",
    "  return a",
    "end",
  })
  vim.api.nvim_buf_set_lines(c_buf, 0, -1, false, {
    "local function my_function()",
    "  local a = 2",
    "  return a",
    "end",
  ...
  local session = {
    parent = { tab = cur_tab, win = p_win, buf = p_buf },
    change = { tab = cur_tab, win = c_win, buf = c_buf },
    hunks = { { old_start = 2, old_count = 1, new_start = 2, new_count = 1 } },
  }

  vim.b[p_buf].oculus_inspect = { file = "test.lua", role = "parent", session = session }
  vim.b[c_buf].oculus_inspect = { file = "test.lua", role = "change", session = session }

  local group = { session }

  vim.api.nvim_win_call(c_win, function()
  ...
  local c_ranges, c_lines = fake_ctx.get(c_win)
  assert(#c_ranges == 1)
  assert(c_ranges[1][1] == 0)
  assert(c_ranges[1][3] == 1)
  assert(#c_lines == 1)
  assert(c_lines[1] == "local function my_function()")
  ...
  package.loaded[ctx_module_name] = original_ctx
end
