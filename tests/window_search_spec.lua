...
window.show_issue_picker({
  ...
  details = {
    number = 42,
    title = "Keep the activity transition visible",
    body = "The activity footer obscures the issue selection query.",
    comment = "Show repository context while choosing relevant files.",
  },
...
assert(issue_picker_text:find(
  "Keep the activity transition visible",
  1,
  true
))
assert(issue_picker_text:find(
  "Show repository context while choosing relevant files.",
  1,
  true
))
...
window.show_issue_picker({
  opts = state.opts,
  info = { owner = "example", repo = "repository", number = 42 },
  details = { number = 42, title = "Cancel this selection" },
  repository = "C:/source/repository",
  default_source = "C:/source",
  remote_url = "https://github.com/example/repository.git",
}, {
  { action = "cancel", label = "Cancel issue inspection" },
}, function(choice)
  if choice.action == "cancel" then
    issue_picker_cancelled = true
    window.restore_issue_picker_page()
  end
end)
back_mapping.callback()
assert(issue_picker_cancelled)
assert(state.view == "activity")
...
window.show_change_picker({
  ...
  change_picker_choice = choice
end)
assert(state.view == "issue_picker")
local change_picker_text = table.concat(
  vim.api.nvim_buf_get_lines(state.buf, 0, -1, false),
  "\n"
)
...
local changed_file_line
for line, target in pairs(state.line_targets) do
  if target.inspection == changed_inspection then
    changed_file_line = line
    break
  end
end
assert(changed_file_line)
vim.api.nvim_win_set_cursor(state.win, { changed_file_line, 0 })
select_mapping.callback()
assert(change_picker_choice.inspection == changed_inspection)
