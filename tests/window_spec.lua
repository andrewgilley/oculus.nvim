...
do
  ...
  state.activity_animated_command = "b"
  state.activity_animation_frame = 1

  local action_called = false
  window._animate_activity_footer_keystroke("b", function()
    action_called = true
  end)

  assert(state.footer_buf ~= nil)
  local frame1_text = table.concat(
    vim.api.nvim_buf_get_lines(state.footer_buf, 0, -1, false),
    "\n"
  )
  ...
  if state.footer_buf and vim.api.nvim_buf_is_valid(state.footer_buf) then
    vim.api.nvim_buf_delete(state.footer_buf, { force = true })
  end
  state.footer_win = nil
  state.footer_buf = nil

  if dummy_win and vim.api.nvim_win_is_valid(dummy_win) then
    vim.api.nvim_win_close(dummy_win, true)
  end
  if dummy_buf and vim.api.nvim_buf_is_valid(dummy_buf) then
